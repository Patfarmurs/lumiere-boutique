...
import { GOLD, IVORY, OBSIDIAN, FONTS, initialCart } from "./constants/theme";
...
export default function App() {
  ...
  const [servicesOpen, setServicesOpen] = useState(null);   // service name string | null
  const [legalOpen,    setLegalOpen]    = useState(null);   // "Privacy"|"Terms"|"Cookies"|null
  ...
  const handleAdd = (id, name, priceStr) => {
    const price = parseInt(priceStr.replace(/\D/g,""), 10);
    setCartList(c => c.find(i => i.id === id) ? c : [...c, { id, name, variant:"One Size", price }]);
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        body { font-family:'Jost',sans-serif; background:${IVORY}; color:${OBSIDIAN}; }
        @keyframes ticker { from{transform:translateX(0)} to{transform:translateX(-50%)} }
        @keyframes fadeUp { from{opacity:0;transform:translateY(28px)} to{opacity:1;transform:translateY(0)} }
        .tk  { animation:ticker 30s linear infinite; display:inline-flex; white-space:nowrap; }
        .fu  { animation:fadeUp 0.85s ease both; }
        .d1  { animation-delay:0.12s; } .d2 { animation-delay:0.22s; }
        .d3  { animation-delay:0.32s; } .d4 { animation-delay:0.42s; }
        ::-webkit-scrollbar       { width:4px; }
        ::-webkit-scrollbar-thumb { background:${GOLD}; border-radius:2px; }
        .col-card:hover .col-cta  { color:${GOLD} !important; gap:18px !important; }
      `}</style>
      ...
      <PaymentModal
        ...
        payStep={payStep}     setPayStep={setPayStep}
