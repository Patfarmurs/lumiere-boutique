      ...
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:620, padding:"0 24px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", borderBottom:`1px solid rgba(184,151,62,0.5)`, paddingBottom:12 }}>
          ...
          <input autoFocus value={query} onChange={e => setQuery(e.target.value)}
            onKeyDown={e => {
              if (e.key === "Enter" && query.trim()) { onClose(); onNav("featured"); setActiveFilter("all"); }
              if (e.key === "Escape") onClose();
            }}
            placeholder="Search garments, collections, designers…"
            style={{ flex:1, background:"none", border:"none", outline:"none", color:IVORY,
              fontFamily:"'Cormorant Garamond',serif", fontSize:"1.55rem", fontWeight:300,
              letterSpacing:"0.02em", caretColor:GOLD }}/>
          <button onClick={onClose}
            style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(249,245,239,0.4)",
              fontSize:"0.7rem", letterSpacing:"0.15em", fontFamily:"'Jost',sans-serif",
              textTransform:"uppercase", marginLeft:14 }}>Esc</button>
        </div>

        <div style={{ display:"flex", gap:10, marginTop:20, flexWrap:"wrap" }}>
          {["New Arrivals","Women","Men","Accessories","Designers","Evening Couture"].map(tag => (
            <button key={tag} onClick={() => handleTag(tag)}
              style={{ padding:"7px 16px", border:"1px solid rgba(249,245,239,0.12)",
