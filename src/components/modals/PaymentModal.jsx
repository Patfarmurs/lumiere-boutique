...
export default function PaymentModal({
  ...
  delivery, setDelivery, card, setCard,
  payMethod, setPayMethod, payErrors, setPayErrors,
  total, cartList, qtys,
  validateDelivery, validateCard, formatCardNumber, formatExpiry,
  onClose, onConfirm,
}) {
  ...
  const labelStyle = { fontSize:"0.6rem", letterSpacing:"0.16em", textTransform:"uppercase", color:WARM_GRAY, display:"block", marginBottom:7 };
  const errStyle   = { fontSize:"0.6rem", color:"#c0392b", marginTop:4 };

  const Field = ({ label, field, value, onChange, placeholder, type="text" }) => (
  ...
  return (
    <div className="payment-overlay">
      <div className="payment-box">
        ...
        {/* Header */}
        <div style={{ padding:"28px 28px 20px", borderBottom:`1px solid rgba(184,151,62,0.14)`,
          ...
          <div>
            ...
            <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(1.2rem,3vw,1.5rem)", fontWeight:300 }}>
              {payStep === 1 ? "Delivery Details" : payStep === 2 ? "Payment" : "Order Confirmed"}
            </h3>
          </div>
          {payStep < 3 && (
            <button onClick={onClose} className="modal-close" style={{ position:"static", color:WARM_GRAY }}>✕</button>
          )}
        </div>
        ...
        <div style={{ padding:"24px 28px 32px", flex:1 }}>
          ...
          {payStep === 1 && (
            <>
              ...
              <div className="payment-fields-row">
                <Field label="Email"  field="email" value={delivery.email}  placeholder="jane@email.com" type="email" onChange={e => setDelivery(d=>({...d,email:e.target.value}))}/>
                <Field label="Phone"  field="phone" value={delivery.phone}  placeholder="+27 82 000 0000"             onChange={e => setDelivery(d=>({...d,phone:e.target.value}))}/>
              </div>
              ...
              <div className="payment-fields-row">
                <Field label="City"        field="city"   value={delivery.city}   placeholder="Cape Town" onChange={e => setDelivery(d=>({...d,city:e.target.value}))}/>
                <Field label="Postal Code" field="postal" value={delivery.postal} placeholder="8001"      onChange={e => setDelivery(d=>({...d,postal:e.target.value}))}/>
              </div>

              {/* Summary */}
              <div style={{ padding:"16px", background:CREAM, marginBottom:24 }}>
                {cartList.map(item => (
                ...
                <div style={{ fontSize:"0.62rem", color:WARM_GRAY, marginTop:6, textAlign:"right" }}>Includes complimentary shipping & gift wrapping</div>
              </div>
            ...
            <>
              ...
              {payMethod === "card" && (
                <>
                  ...
                  <div style={{ display:"flex", gap:8, marginBottom:20, alignItems:"center", flexWrap:"wrap" }}>
                    ...
                    <span style={{ fontSize:"0.6rem", color:"rgba(107,99,89,0.5)" }}>🔒 SSL Secured</span>
                  </div>
                </>
              )}
              ...
              {payMethod === "eft" && (
                <div style={{ background:CREAM, padding:"20px", marginBottom:20 }}>
                  ...
                  <p style={{ fontSize:"0.72rem", color:WARM_GRAY, marginTop:12, lineHeight:1.7 }}>
                    Please use your reference when making the transfer. Your order will be confirmed within 24 hours.
                  </p>
                </div>
              )}
              ...
              {payMethod === "payfast" && (
                <div style={{ background:CREAM, padding:"24px", marginBottom:20, textAlign:"center" }}>
                  ...
                  <p style={{ fontSize:"0.78rem", color:WARM_GRAY, lineHeight:1.7 }}>
                    You'll be securely redirected to PayFast. Supports credit/debit cards, Instant EFT, Mobicred, and more.
                  </p>
                </div>
              )}
              ...
              <div style={{ display:"flex", gap:12 }}>
                <button onClick={() => { setPayErrors({}); setPayStep(1); }}
                  style={{ padding:"15px 20px", background:"transparent", border:`1px solid rgba(0,0,0,0.2)`,
                ...
                >← Back</button>
                <button onClick={() => { const ok = payMethod==="card" ? validateCard() : true; if(ok){ setPayErrors({}); onConfirm(); } }}
          ...
          {payStep === 3 && (
            <div style={{ textAlign:"center", padding:"16px 0 8px" }}>
              ...
              <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(1.6rem,3vw,2rem)",
                fontWeight:300, color:OBSIDIAN, marginBottom:12 }}>Order Confirmed</h3>
              <p style={{ fontSize:"0.84rem", fontWeight:300, color:WARM_GRAY, lineHeight:1.9, maxWidth:360, margin:"0 auto 8px" }}>
                Thank you, {delivery.name.split(" ")[0] || "valued client"}. Your order has been received and will be dispatched within 2–3 business days.
              </p>
              ...
              <div style={{ background:CREAM, padding:"16px 20px", marginBottom:32, textAlign:"left" }}>
                ...
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"1.1rem", fontWeight:300 }}>LUM-{Date.now().toString().slice(-8)}</div>
                <div style={{ fontSize:"0.72rem", color:WARM_GRAY, marginTop:4 }}>Delivering to {delivery.address}, {delivery.city}</div>
              </div>
              <button onClick={onClose} style={{
                padding:"14px 48px", background:OBSIDIAN, border:"none", color:IVORY,
                fontFamily:"'Jost',sans-serif", fontSize:"0.67rem", fontWeight:500,
                letterSpacing:"0.2em", textTransform:"uppercase", cursor:"pointer", transition:"background 0.3s" }}
                onMouseEnter={e => e.currentTarget.style.background=GOLD}
                onMouseLeave={e => e.currentTarget.style.background=OBSIDIAN}
