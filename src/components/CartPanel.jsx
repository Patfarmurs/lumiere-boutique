...
export default function CartPanel({ cartOpen, cartList, qtys, total, onClose, onQty, onRemove, onCheckout }) {
  return (
    <>
      {cartOpen && <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,11,9,0.7)", backdropFilter:"blur(4px)", zIndex:200 }}/>}

      <div className={`cart-panel ${cartOpen ? "open" : "closed"}`}>
        {/* Header */}
        <div style={{ padding:"36px 36px 20px", borderBottom:`1px solid rgba(184,151,62,0.14)`,
          ...
          <button onClick={onClose} style={{ background:"none", border:"none", fontSize:"1.1rem",
            cursor:"pointer", color:WARM_GRAY, transition:"transform 0.3s" }}
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"26px 36px" }}>
          {cartList.length === 0 ? (
            ...
            <div key={item.id} style={{ display:"grid", gridTemplateColumns:"72px 1fr auto",
              ...
              {/* ── Thumbnail — shows real image if available, placeholder if not ── */}
              <div style={{ width:72, height:90, flexShrink:0, overflow:"hidden" }}>
                {item.image ? (
                  <img
                    src={item.image}
                    alt={item.name}
              ...
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  {[["−", Math.max(1,(qtys[item.id]||1)-1)],["+", Math.min(10,(qtys[item.id]||1)+1)]].map(([sym,val]) => (
                    <button key={sym} onClick={() => onQty(item.id, val)}
                    ...
                    >{sym}</button>
                  )).reduce((acc, btn, i) => i===0 ? [btn] : [...acc, <span key="q" style={{fontSize:"0.86rem",minWidth:14,textAlign:"center"}}>{qtys[item.id]||1}</span>, btn], [])}
                </div>
              </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8 }}>
                ...
                <button onClick={() => onRemove(item.id)} title="Remove"
                  style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(107,99,89,0.4)", transition:"color 0.2s", padding:0 }}
